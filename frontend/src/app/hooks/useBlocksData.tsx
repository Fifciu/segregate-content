...
export function BlocksProvider({ children }: { children: React.ReactNode }) {
  ...
  function prepareDaysForCopy() {
    return days.map(day => {
      return {
        blocks: Object.values(day.blocks).map(block => {
          return {
            name: block.name,
  ...
  return (
    <BlocksContext.Provider value={{ days, setBlock, setDays, prepareDaysForCopy }}>
      {children}
